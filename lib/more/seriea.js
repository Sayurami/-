...
async function saStandings() {
    const data = await fetchFootballData('competitions/SA/standings');
    ...
    return {
        competition,
        standings: standings.slice(0, 20).map((team) => ({
...
// Get Serie A Matches
async function saMatches() {
    const data = await fetchFootballData('competitions/SA/matches');
    const competition = data.competition?.name || 'Unknown Competition';
...
async function saTopScorers() {
    const data = await fetchFootballData('competitions/SA/scorers');
    const competition = data.competition?.name || 'Unknown Competition';
    ...
    return {
        competition,
        topScorers: scorers.slice(0, 20).map((scorer, index) => ({
            rank: index + 1,
...
// Get Serie A Upcoming Matches
async function saUpcomingMatches() {
    const data = await fetchFootballData('competitions/SA/matches');
    const competition = data.competition?.name || 'Unknown Competition';
    ...
    return {
        competition,
        upcomingMatches: upcomingMatches.slice(0, 20).map((match) => ({
            matchday: match.matchday || 'N/A',
