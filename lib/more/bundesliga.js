...
async function bl1Standings() {
    const data = await fetchFootballData('competitions/BL1/standings');
    ...
    return {
        competition,
        standings: standings.slice(0, 18).map((team) => ({
...
async function bl1Matches() {
    const data = await fetchFootballData('competitions/BL1/matches');
    const competition = data.competition?.name || 'Unknown Competition';
...
async function bl1TopScorers() {
    const data = await fetchFootballData('competitions/BL1/scorers');
    const competition = data.competition?.name || 'Unknown Competition';
...
// Get Bundesliga Upcoming Matches
async function bl1UpcomingMatches() {
    const data = await fetchFootballData('competitions/BL1/matches');
    const competition = data.competition?.name || 'Unknown Competition';
