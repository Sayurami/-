...
// Get European Championship Standings
async function ecStandings() {
    const data = await fetchFootballData('competitions/EC/standings');
    ...
    return {
        competition,
        standings: standings.slice(0, 24).map((team) => ({
...
// Get European Championship Matches
async function ecMatches() {
    ...
    return {
        competition,
        matches: matches.slice(0, 24).map((match) => ({
...
// Get European Championship Top Scorers
async function ecTopScorers() {
    const data = await fetchFootballData('competitions/EC/scorers');
    const competition = data.competition?.name || 'Unknown Competition';
    ...
    return {
        competition,
        topScorers: scorers.slice(0, 24).map((scorer, index) => ({
            rank: index + 1,
...
// Get European Championship Upcoming Matches
async function ecUpcomingMatches() {
    const data = await fetchFootballData('competitions/EC/matches');
    const competition = data.competition?.name || 'Unknown Competition';
    ...
    return {
        competition,
        upcomingMatches: upcomingMatches.slice(0, 24).map((match) => ({
            matchday: match.matchday || 'N/A',
